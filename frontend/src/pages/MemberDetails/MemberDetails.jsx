...
export default function MemberDetails() {
  ...
  return (
    <div className='w-3/4 text-black p-3'>
        <div onClick={()=> {navigate(-1)}} className='border-2 cursor-pointer w-fit text-xl font-sans text-white p-2 rounded-xl bg-slate-900'>
        ...
        <div className='mt-10 p-2'>
              <div className='w-[100%] h-fit flex'>
                <div className='w-1/3 mx-auto'>
                      <img src={data?.profilePic} alt="" className=''/>
                </div>
                ...
                <div className='mt-1 mb=2 flex gap-4 text-2xl font-semibold'> Status : <Switch onColor='#6366F1' checked={status === "Active"} onChange={()=>{handleSwitcbtn()}}/></div>
                
                {isDateInPast(data?.nextBillDate) && <div onClick = {()=>(setRenew(prev => !prev))}className={`mt-1 rounded-lg p-3 ${renew && status === "Active"? "bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 text-white" :null} border-2 border-slate-900 text-center w-full md:w-1/2 cursor-pointer hover:text-white hover:bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 `}>Renew</div>}
                     ...
                     <div className="w-full">
                        <div className="my-5">
                            ...
                            <select value={planMember} onChange={handleOnChangeSelect} className='w-full border-2 p-2 rounded-lg'>
                                ...
                                <option>2 Months Membership</option> */}
                            </select>
